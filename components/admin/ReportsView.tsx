    ...
    <div>
      ...
      {filtered.length === 0 ? (
        <div style={{ background: '#fff', borderRadius: 14, padding: '60px 20px', textAlign: 'center', boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
          ...
          <div style={{ fontFamily: 'var(--font-nunito)', fontWeight: 800, color: '#aaa' }}>No {filter} reports</div>
        </div>
      ) : (
            ...
            <tbody>
              {filtered.map(r => (
                <tr key={r.id} style={{ borderBottom: '1px solid #f9f9f9' }}>
                  ...
                  <td style={{ padding: '10px 14px', fontFamily: 'var(--font-nunito)', fontSize: 12, color: '#888' }}>
                    {r.reported_by?.full_name ?? '—'}
                  </td>
                  <td style={{ padding: '10px 14px', fontFamily: 'var(--font-nunito)', fontSize: 11, color: '#aaa' }}>
                    ...
                    {r.status === 'open' && (
                      <div style={{ display: 'flex', gap: 6 }}>
                        <button onClick={() => dismiss(r.id)} style={{
                          ...
                          fontFamily: 'var(--font-nunito)', fontSize: 11, fontWeight: 700, cursor: 'pointer', color: '#888',
                        }}>Dismiss</button>
                        <button onClick={() => takeDown(r.id, r.listing_id)} style={{
                          padding: '4px 10px', borderRadius: 6, border: 'none',
                          background: '#ef4444', color: '#fff',
                          fontFamily: 'var(--font-nunito)', fontSize: 11, fontWeight: 700, cursor: 'pointer',
                        }}>Remove</button>
                      </div>
                    )}
                  </td>
