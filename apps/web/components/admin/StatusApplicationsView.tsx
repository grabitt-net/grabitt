      ...
      {apps.map(a => {
        ...
        return (
          <div key={a.id} style={{ background: '#fff', border: '1px solid #eee', borderRadius: 12, padding: 14, marginBottom: 10 }}>
            ...
            {a.evidenceUrl && <a href={/^https?:\/\//i.test(a.evidenceUrl) ? a.evidenceUrl : `/api/status-doc?id=${a.id}`} target="_blank" rel="noreferrer" style={{ display: 'inline-block', marginTop: 6, fontFamily: 'var(--font-ui)', fontSize: 12, fontWeight: 700, color: 'var(--orange)' }}>📎 {/^https?:\/\//i.test(a.evidenceUrl) ? 'View evidence' : 'View proof document'}</a>}
