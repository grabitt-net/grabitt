    ...
    <div>
      ...
      {showAdd && (
        <div style={{ background: '#fff', borderRadius: 14, padding: 18, marginBottom: 16, boxShadow: '0 2px 12px rgba(0,0,0,0.08)' }}>
          <h3 style={{ fontFamily: 'var(--font-ui)', fontWeight: 800, marginBottom: 12 }}>Add Banner Slot</h3>
            ...
            {[
              ...
              <div key={key}>
                ...
                <input type={type} value={(form as any)[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                  style={{ width: '100%', padding: '7px 10px', border: '1.5px solid #e5e7eb', borderRadius: 8, fontFamily: 'var(--font-ui)', fontSize: 12, boxSizing: 'border-box' }} />
              </div>
            ))}
            <div style={{ gridColumn: '1/-1' }}>
              <label style={{ display: 'block', fontSize: 9, fontWeight: 800, color: '#aaa', fontFamily: 'var(--font-ui)', marginBottom: 3, textTransform: 'uppercase', letterSpacing: 0.5 }}>Rented To</label>
              <select value={form.rented_to} onChange={e => setForm(f => ({ ...f, rented_to: e.target.value }))}
                style={{ width: '100%', padding: '7px 10px', border: '1.5px solid #e5e7eb', borderRadius: 8, fontFamily: 'var(--font-ui)', fontSize: 12 }}>
          ...
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 12 }}>
            ...
            <button onClick={addBanner} disabled={saving || !form.slot} style={{ padding: '7px 18px', borderRadius: 50, border: 'none', background: '#FF4500', color: '#fff', fontFamily: 'var(--font-ui)', fontWeight: 800, fontSize: 12, cursor: 'pointer', opacity: saving || !form.slot ? 0.6 : 1 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12 }}>
        {banners.map(b => (
          <div key={b.id} style={{
            ...
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 8 }}>
              <div>
                <div style={{ fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: 12, color: '#FF4500', textTransform: 'uppercase', letterSpacing: 0.5 }}>{b.slot}</div>
                <div style={{ fontFamily: 'var(--font-ui)', fontWeight: 700, fontSize: 13, marginTop: 2 }}>{b.title ?? '—'}</div>
              </div>
              <button onClick={() => toggle(b.id, b.active)} style={{
                padding: '3px 10px', borderRadius: 50, border: 'none', cursor: 'pointer', fontSize: 10, fontWeight: 800, fontFamily: 'var(--font-ui)',
                background: b.active ? '#f0faf4' : '#f5f5f5', color: b.active ? '#16a34a' : '#aaa',
              }}>{b.active ? 'Live' : 'Off'}</button>
            </div>
            {b.crm_contacts && (
              <div style={{ fontSize: 10, color: '#888', fontFamily: 'var(--font-ui)', marginBottom: 4 }}>
                📌 {b.crm_contacts.name} — {b.crm_contacts.company}
              </div>
            )}
            {b.monthly_rate && (
              <div style={{ fontFamily: 'var(--font-ui)', fontWeight: 900, fontSize: 14, color: '#16a34a' }}>
                €{Number(b.monthly_rate).toLocaleString()}/mo
              </div>
            )}
            {(b.starts_at || b.ends_at) && (
              <div style={{ fontSize: 9, color: '#aaa', marginTop: 4, fontFamily: 'var(--font-ui)' }}>
        ...
        {banners.length === 0 && (
          <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px 20px', color: '#ccc', fontFamily: 'var(--font-ui)', fontWeight: 800 }}>
            No banner slots yet
          </div>
        )}
      </div>
