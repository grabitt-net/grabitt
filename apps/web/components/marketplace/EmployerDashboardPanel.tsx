...
// Employer Dashboard — replicates the V20 openEmployerDashboard flow against real
// data: listing counter, messages shortcut, share prompt, and the employer's job
// listings with status + applicant counts and per-listing actions.
...
export default function EmployerDashboardPanel({ onClose, openPanel }: { onClose: () => void; openPanel: (id: PanelId, data?: Record<string, unknown>) => void }) {
    ...
    <div onClick={onClose} className="panel-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 400 }}>
      <div onClick={e => e.stopPropagation()} className="panel-sheet" style={{ background: '#fff', maxHeight: '88vh', display: 'flex', flexDirection: 'column' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 16px 12px', borderBottom: '1px solid #f0f0f0', flexShrink: 0 }}>
          <span style={{ fontFamily: 'var(--font-ui)', fontSize: 15, fontWeight: 900, color: '#1a1a1a' }}>🏢 Employer Dashboard</span>
        ...
        <div style={{ overflowY: 'auto', padding: 16, flex: 1 }}>
          ...
          <div style={{ background: 'linear-gradient(135deg,#1a1a1a,#333)', borderRadius: 14, padding: 16, marginBottom: 14 }}>
            ...
            <a href="/jobs/new" onClick={onClose} style={{ textDecoration: 'none' }}>
            ...
            <div onClick={() => { onClose(); openPanel('verifyMe') }} style={{ marginTop: 8, textAlign: 'center', fontSize: 10, color: 'rgba(255,255,255,0.6)', fontFamily: 'var(--font-ui)', fontWeight: 700, cursor: 'pointer' }}>🏢 Company details &amp; verification →</div>
          </div>

          {/* Messages */}
          <a href="/messages" onClick={onClose} style={{ textDecoration: 'none' }}>
          ...
          <div onClick={() => { onClose(); openPanel('findStaff') }} style={{ display: 'flex', alignItems: 'center', gap: 10, background: '#f8f9fa', border: '1px solid #eee', borderRadius: 12, padding: 12, marginBottom: 14, cursor: 'pointer' }}>
            <div style={{ fontSize: 22 }}>🧑‍💼</div>
            <div style={{ flex: 1 }}>
                  ...
                  return (
                    <div key={j.id} style={{ background: '#f8f9fa', border: '1px solid #1a1a1a', borderRadius: 12, padding: '11px 12px' }}>
                      ...
                      <div style={{ marginTop: 8, display: 'flex', gap: 6 }}>
                        <button onClick={() => { onClose(); openPanel('applications', { jobId: j.id }) }} style={{ flex: 1, background: '#1a1a1a', color: '#fff', border: 'none', borderRadius: 50, padding: 8, fontFamily: 'var(--font-ui)', fontSize: 10, fontWeight: 800, cursor: 'pointer' }}>📋 Applicants{newCount ? ` (${newCount} new)` : ''}</button>
                        <a href={`/jobs/new?edit=${j.listingId}`} onClick={onClose} style={{ flex: 1, textDecoration: 'none' }}>
