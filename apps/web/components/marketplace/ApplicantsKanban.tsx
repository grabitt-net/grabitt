...
export default function ApplicantsKanban({ jobId, onClose, openPanel }: { jobId: string; onClose: () => void; openPanel: (id: PanelId, data?: Record<string, unknown>) => void }) {
  ...
  const move = async (a: App, toStatus: string) => {
    ...
    if (toStatus === 'rejected_pre') {
      const reason = prompt(t('Add a short reason for rejecting this candidate (kept on file):'))
  ...
  const saveNote = async (a: App) => {
    ...
    try {
      ...
      toast(`✓ ${t('Note saved')}`)
    } catch (e: any) { toast(e?.message || t('Could not save the note.')) }
    finally { setSaving(null) }
  }
    ...
    <div onClick={onClose} className="panel-overlay" style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', zIndex: 400 }}>
      <div onClick={e => e.stopPropagation()} className="panel-sheet" style={{ position: 'relative', background: '#f4f6f8', height: '85vh', maxHeight: '90vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', background: '#fff', borderBottom: '1px solid #e8e8e8', flexShrink: 0 }}>
          <button onClick={onClose} style={{ display: 'inline-flex', alignItems: 'center', gap: 4, background: '#f5f0e8', color: 'var(--dark)', border: 'none', borderRadius: 50, padding: '7px 13px', fontFamily: 'var(--font-nunito)', fontSize: 12.5, fontWeight: 900, cursor: 'pointer', flexShrink: 0 }}>{t('Back to jobs')}</button>
          <div style={{ minWidth: 0, flex: 1 }}>
            <div style={{ fontFamily: 'var(--font-nunito)', fontSize: 14.5, fontWeight: 900, color: 'var(--dark)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{job ? job.jobTitle : t('Applicants')}</div>
            <div style={{ fontFamily: 'var(--font-nunito)', fontSize: 11, color: '#888' }}>{job ? `${job.applications.length} ${t(job.applications.length === 1 ? 'applicant' : 'applicants')}` : ''}</div>
          </div>
        ...
        {!loaded ? (
          <div style={{ textAlign: 'center', padding: 50, color: '#888', fontFamily: 'var(--font-ui)', fontSize: 13 }}>{t('Loading…')}</div>
        ) : !job ? (
          <div style={{ textAlign: 'center', padding: 50, color: '#888', fontFamily: 'var(--font-ui)', fontSize: 13 }}>{t('Job not found.')}</div>
        ) : (
                ...
                return (
                  <div key={stage.key} style={{ flex: '0 0 240px', display: 'flex', flexDirection: 'column', background: '#eceff3', borderRadius: 12, maxHeight: '100%' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px', borderBottom: `2px solid ${stage.color}`, flexShrink: 0 }}>
                      <span style={{ fontFamily: 'var(--font-ui)', fontSize: 12.5, fontWeight: 900, color: '#1a1a1a', flex: 1 }}>{t(stage.label)}</span>
                          ...
                          <div key={a.id} style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 10, padding: 10, boxShadow: '0 1px 3px rgba(0,0,0,0.05)', opacity: saving === a.id ? 0.6 : 1 }}>
                            <div onClick={() => { setDetailId(a.id); setNoteDraft(a.employerNote ?? '') }} style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                              ...
                              <div style={{ flex: 1, minWidth: 0 }}>
                                ...
                                <div style={{ fontFamily: 'var(--font-ui)', fontSize: 9.5, color: '#999' }}>{t('Applied')} {new Date(a.createdAt).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</div>
                              </div>
                            ...
                            <div style={{ display: 'flex', gap: 6, marginTop: 8, alignItems: 'center' }}>
                              ...
                              <button onClick={() => message(a)} title={t('Message candidate')} style={miniBtn(true)}>💬</button>
                            </div>
                          </div>
                        )
                      })}
        ...
        {detail && (
          <div onClick={() => setDetailId(null)} style={{ position: 'absolute', inset: 0, background: 'rgba(0,0,0,0.4)', display: 'flex', justifyContent: 'flex-end', zIndex: 10 }}>
            <div onClick={e => e.stopPropagation()} style={{ width: 'min(420px, 100%)', height: '100%', background: '#fff', overflowY: 'auto', padding: 18, boxShadow: '-4px 0 20px rgba(0,0,0,0.15)' }}>
              ...
              <button onClick={() => message(detail)} style={{ width: '100%', background: 'linear-gradient(135deg,var(--orange),var(--orange2,#ff8a3d))', color: '#fff', border: 'none', borderRadius: 10, padding: 11, fontFamily: 'var(--font-ui)', fontSize: 13, fontWeight: 900, cursor: 'pointer', marginBottom: 12 }}>💬 {t('Message this candidate')}</button>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, marginBottom: 12 }}>
                {detail.fullName && <D label={t('Name')} value={detail.fullName} />}
                {detail.email && <D label={t('Email')} value={detail.email} />}
                {detail.phone && <D label={t('Phone')} value={detail.phone} />}
                {detail.currentRole && <D label={t('Current role')} value={detail.currentRole} />}
                {expLabel(detail.experienceMonths) && <D label={t('Experience')} value={expLabel(detail.experienceMonths)!} />}
                {detail.languages.length > 0 && <D label={t('Languages')} value={detail.languages.join(', ')} />}
                {detail.location && <D label={t('Location')} value={detail.location} />}
                {detail.rightToWork && <D label={t('Right to work')} value={detail.rightToWork} />}
                {detail.availability && <D label={t('Availability')} value={detail.availability} />}
                {detail.expectedSalary != null && <D label={t('Expected salary')} value={`€${detail.expectedSalary.toLocaleString()}/mo`} />}
                <a href={`/api/cv-pdf?applicationId=${detail.id}`} target="_blank" rel="noreferrer" style={{ fontFamily: 'var(--font-ui)', fontSize: 12, fontWeight: 800, color: ORANGE }}>📄 {t('View CV')}</a>
                {detail.cvUrl && <a href={`/api/cv?applicationId=${detail.id}`} target="_blank" rel="noreferrer" style={{ fontFamily: 'var(--font-ui)', fontSize: 12, fontWeight: 800, color: '#555' }}>📎 {t('Attached file')}</a>}
              </div>
              {Object.keys(detail.answers).length > 0 && (
                <div style={{ marginBottom: 12 }}>
                  <div style={{ fontFamily: 'var(--font-ui)', fontSize: 11, fontWeight: 900, color: '#555', marginBottom: 4 }}>{t('Screening answers')}</div>
              ...
              <div style={{ fontFamily: 'var(--font-ui)', fontSize: 11, fontWeight: 900, color: '#555', marginBottom: 4 }}>{t('Private note')}</div>
              <textarea value={noteDraft} onChange={e => setNoteDraft(e.target.value)} rows={3} placeholder={t("Your notes on this candidate (never shown to them)…")} style={{ width: '100%', boxSizing: 'border-box', border: '1.5px solid #e5e7eb', borderRadius: 8, padding: 8, fontFamily: 'var(--font-ui)', fontSize: 12, resize: 'vertical' }} />
              <button onClick={() => saveNote(detail)} style={{ marginTop: 6, background: '#1a1a1a', color: '#fff', border: 'none', borderRadius: 8, padding: '8px 14px', fontFamily: 'var(--font-ui)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>{t('Save note')}</button>
            </div>
          </div>
        )}
      </div>
