      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', width: '100%', maxWidth: 520, borderRadius: '20px 20px 0 0', padding: 20 }}>
        {sent ? (
          ...
          <>
            ...
            <button onClick={send} disabled={sending || !body.trim()} style={{ width: '100%', marginTop: 12, background: !body.trim() ? '#ccc' : 'linear-gradient(135deg,var(--orange),var(--orange2))', color: '#fff', border: 'none', borderRadius: 14, padding: 14, fontFamily: 'var(--font-nunito)', fontSize: 14, fontWeight: 900, cursor: sending ? 'wait' : 'pointer' }}>{sending ? t('Sending…') : `${t('Send message')}`}</button>
