...
const PropertyMap = dynamic(() => import('@/components/marketplace/PropertyMap'), {
  ssr: false,
  loading: () => <div style={{ width: '100%', height: 420, borderRadius: 14, background: '#ece3d7', display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: 'var(--font-nunito)', color: '#a99', fontSize: 13 }}>{t('Loading map…')}</div>,
})

const RADII = [5, 10, 25, 50, 0]
const radiusLabel = (km: number) => (km === 0 ? t('Whole island') : `${km} ${t('km')}`)
...
export default function PropertyPage() {
  ...
  const locateMe = useCallback(() => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { setLocating(false); toast(t('Could not get your location. Please allow location access.')) },
  ...
  const toggleLocation = (loc: string) => setLocations(prev => prev.includes(loc) ? prev.filter(x => x !== loc) : [...prev, loc])
  ...
  return (
    ...
    <main className="app-shell" style={{ background: 'var(--cream)', minHeight: '100vh', paddingBottom: 40, boxShadow: '0 0 40px rgba(0,0,0,0.06)' }}>
      <Topbar title={t('Property')} />
      <QuickActions belowPromo={heroBanner
        ? <HeroBanner src={heroBanner} alt={t('Property')} />
        : <PageHero title={t('Property')} tagline={t('Find your place in the sun.')} body={t('Buying, selling, renting, or letting — your next home or investment is right here on the islands. Browse houses, apartments and holiday lets across the Canaries, or list your own property to reach local buyers and tenants directly. No jargon, no faff — just island living made simple.')} />} />
      ...
      <header style={{ background: 'var(--sand)', padding: '12px 14px', borderBottom: '1.5px solid var(--sand2)' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <div style={{ flex: 1, minWidth: 0, display: 'flex', gap: 6, overflowX: 'auto', scrollbarWidth: 'none' }}>
            {TYPES.map(ty => (
              <button key={ty.label} onClick={() => setType(ty.value ?? '')} style={{
                flex: '0 0 auto', border: `1.5px solid ${type === (ty.value ?? '') ? 'var(--orange)' : '#e5dccd'}`,
                background: type === (ty.value ?? '') ? 'var(--orange)' : '#fff', color: type === (ty.value ?? '') ? '#fff' : '#555',
                borderRadius: 50, padding: '6px 14px', fontFamily: 'var(--font-nunito)', fontSize: 12, fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap',
              }}>{t(ty.label)}</button>
            ))}
          </div>
          <Link href="/property/new" style={{ flexShrink: 0, textDecoration: 'none', background: 'var(--orange)', color: '#fff', borderRadius: 50, padding: '8px 16px', fontFamily: 'var(--font-nunito)', fontSize: 12, fontWeight: 800, whiteSpace: 'nowrap' }}>{t('List a Property')}</Link>
        </div>
        ...
        <form onSubmit={e => { e.preventDefault(); run() }} style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'center' }}>
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder={t('🔍 Search property…')} style={{ ...inp, flex: 1, minWidth: 150, padding: '7px 11px' }} />
          <input value={minPrice} onChange={e => setMinPrice(e.target.value)} inputMode="numeric" placeholder={t('Min €')} style={{ ...inp, width: 78, padding: '7px 10px' }} />
          <input value={maxPrice} onChange={e => setMaxPrice(e.target.value)} inputMode="numeric" placeholder={t('Max €')} style={{ ...inp, width: 78, padding: '7px 10px' }} />
          <select value={minBedrooms} onChange={e => setMinBedrooms(e.target.value)} style={{ ...sel, padding: '7px 8px' }}>
            <option value="">{t('Beds')}</option>{[1, 2, 3, 4, 5].map(n => <option key={n} value={n}>{n}+ {t('beds')}</option>)}
          </select>
          <select value={minBathrooms} onChange={e => setMinBathrooms(e.target.value)} style={{ ...sel, padding: '7px 8px' }}>
            <option value="">{t('Baths')}</option>{[1, 2, 3].map(n => <option key={n} value={n}>{n}+ {t('baths')}</option>)}
          </select>
          <button type="submit" style={{ ...btn, padding: '7px 16px' }}>{t('Search')}</button>
        </form>

        {/* Multi-select area pills */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 8 }}>
          {GC_TOWNS.map(town => {
            const on = locations.includes(town)
            return (
              <button key={town} onClick={() => toggleLocation(town)} style={{
          ...
          {locations.length > 0 && <button onClick={() => setLocations([])} style={{ border: 'none', background: 'none', color: '#1a1a1a', fontFamily: 'var(--font-nunito)', fontSize: 11, fontWeight: 800, cursor: 'pointer' }}>{t('Clear areas')}</button>}
        </div>
          ...
          {FILTERABLE_FEATURES.map(f => {
            ...
            return (
              <button key={f} onClick={() => toggleFeature(f)} style={{
                ...
                fontSize: 11, fontWeight: 800, cursor: 'pointer', whiteSpace: 'nowrap',
              }}>{featureIcon(f)} {t(featureLabel(f))}</button>
            )
          })}
        </div>
      </header>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap', padding: '10px 14px' }}>
        <span style={{ fontFamily: 'var(--font-nunito)', fontSize: 12, color: '#888', fontWeight: 700 }}>
          {loading ? t('Searching…') : `${displayed.length} ${t(displayed.length === 1 ? 'property' : 'properties')}`}
          {nearActive && centre && radius !== 0 && ` ${t('within')} ${radiusLabel(radius)}`}
        </span>
        <button onClick={locateMe} disabled={locating} style={{ background: nearActive ? 'var(--orange)' : '#fff', color: nearActive ? '#fff' : 'var(--orange)', border: '1.5px solid var(--orange)', borderRadius: 50, padding: '7px 12px', fontFamily: 'var(--font-nunito)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
          📍 {locating ? t('Locating…') : nearActive ? t('Near me') : t('Locate me')}
        </button>
        {nearActive && (
        ...
        {nearActive && <button onClick={() => { setNearActive(false); setCentre(null) }} style={{ background: 'none', border: 'none', color: '#888', fontFamily: 'var(--font-nunito)', fontSize: 11.5, fontWeight: 800, cursor: 'pointer' }}>{t('Clear')}</button>}
        <div style={{ marginLeft: 'auto', display: 'flex', background: '#fff', border: '1px solid #e5dccd', borderRadius: 50, overflow: 'hidden' }}>
          <button onClick={() => setView('list')} style={{ border: 'none', background: view === 'list' ? 'var(--orange)' : 'transparent', color: view === 'list' ? '#fff' : '#888', padding: '6px 16px', fontFamily: 'var(--font-nunito)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>{t('List')}</button>
          <button onClick={() => setView('map')} style={{ border: 'none', background: view === 'map' ? 'var(--orange)' : 'transparent', color: view === 'map' ? '#fff' : '#888', padding: '6px 16px', fontFamily: 'var(--font-nunito)', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>{t('Map')}</button>
        </div>
      </div>
        ...
        <div style={{ padding: '0 14px', maxWidth: 760, margin: '0 auto' }}>
          ...
          {points.length < displayed.length && (
            <div style={{ fontFamily: 'var(--font-nunito)', fontSize: 11, color: '#aaa', textAlign: 'center', padding: '8px 0' }}>
              {`${displayed.length - points.length} ${t(displayed.length - points.length === 1 ? 'property' : 'properties')} ${t('without a mappable location aren’t shown on the map.')}`}
            </div>
          )}
        </div>
      ) : (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12, padding: '0 14px', maxWidth: 760, margin: '0 auto' }}>
        {displayed.map(p => {
          ...
          const termLabel = ({ short_term: t('Short-term'), long_term: t('Long-term'), holiday: t('Holiday rental') } as Record<string, string>)[p.rentalTerm as string]
            ...
            <Link key={p.id} href={`/listings/${l.id}`} style={{ textDecoration: 'none' }}>
              ...
              <div style={{ display: 'flex', height: 150, background: '#fff', borderRadius: 14, border: '1px solid #ece3d7', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ position: 'relative', flex: '0 0 42%', maxWidth: 220, background: 'var(--sand)' }}>
                  ...
                  <span style={{ position: 'absolute', top: 8, left: 8, background: 'rgba(26,26,26,0.85)', color: '#fff', fontFamily: 'var(--font-nunito)', fontSize: 9, fontWeight: 900, padding: '3px 9px', borderRadius: 50, textTransform: 'uppercase', letterSpacing: 0.3 }}>{t(PROP_LABEL[p.type] ?? p.type)}</span>
                ...
                <div style={{ flex: 1, minWidth: 0, padding: '11px 12px', display: 'flex', flexDirection: 'column' }}>
                  ...
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px 12px', fontFamily: 'var(--font-nunito)', fontSize: 12, fontWeight: 700, color: '#1a1a1a', marginTop: 6 }}>
                    {p.bedrooms > 0 && <span>🛏 {p.bedrooms} {t(p.bedrooms === 1 ? 'bed' : 'beds')}</span>}
                    {p.bathrooms > 0 && <span>🚿 {p.bathrooms} {t(p.bathrooms === 1 ? 'bath' : 'baths')}</span>}
                    {p.m2 && <span>📐 {Number(p.m2)} m²</span>}
                    {p.floor != null && <span>🏢 {t('Floor')} {p.floor}</span>}
                  </div>
        ...
        {!loading && displayed.length === 0 && (
          <div style={{ textAlign: 'center', padding: 50, fontFamily: 'var(--font-nunito)', color: '#bbb' }}>
            ...
            <div style={{ fontSize: 15, fontWeight: 800 }}>{t('No property matches your search')}</div>
            {nearActive && <div style={{ fontSize: 12, marginTop: 4 }}>{t('Try widening the search area.')}</div>}
          </div>
        )}
      </div>
      )}
