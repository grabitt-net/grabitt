...
export default function DirectoryPage() {
  ...
  // Auto-translate the business-written card text for non-English viewers.
  const trNames = useTranslated(shown.map(l => l.name))
  const trCats = useTranslated(shown.map(l => l.category ?? ''))
  const trDescs = useTranslated(shown.map(l => l.description ?? ''))

  return (
    <PanelProvider>
      <main className="app-shell" style={{ background: 'var(--cream)', minHeight: '100vh', paddingBottom: 60, boxShadow: '0 0 40px rgba(0,0,0,0.06)' }}>
        <Topbar title={t('Business Directory')} />
        <QuickActions />
        ...
        <div style={{ maxWidth: 960, margin: '0 auto', padding: '16px 14px' }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
            <label style={{ flex: '1 1 200px', minWidth: 160 }}>
              <span style={filterLbl}>{t('Business type')}</span>
              <select value={cat} onChange={e => setCat(e.target.value)} style={filterSel}>
                {categories.map(c => <option key={c} value={c}>{c === 'All' ? t('All business types') : c}</option>)}
              </select>
            </label>
            <label style={{ flex: '1 1 200px', minWidth: 160 }}>
              <span style={filterLbl}>{t('Location')}</span>
              <select value={loc} onChange={e => setLoc(e.target.value)} style={filterSel}>
                {locations.map(c => <option key={c} value={c}>{c === 'All' ? t('All locations') : c}</option>)}
              </select>
            </label>
          </div>

          {listings === null ? (
            <div style={{ padding: 50, textAlign: 'center', fontFamily: 'var(--font-nunito)', color: '#1a1a1a' }}>{t('Loading…')}</div>
          ) : shown.length === 0 ? (
            <div style={{ padding: 50, textAlign: 'center', fontFamily: 'var(--font-nunito)', color: '#aaa' }}>
              <div style={{ fontSize: 40, marginBottom: 10 }}>📒</div>
              <div style={{ fontSize: 15, fontWeight: 800 }}>{t('No businesses listed yet')}</div>
            </div>
          ) : (
              ...
              {shown.map((l, idx) => (
                <Link key={l.id} href={`/directory/${l.slug || l.id}`} style={{ textDecoration: 'none', minWidth: 0 }}>
                  <div style={{ background: '#fff', border: '1px solid #ece3d7', borderRadius: 14, overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', height: '100%' }}>
                    <div style={{ height: 90, background: '#fff', display: 'flex', alignItems: 'center', justifyContent: 'center', overflow: 'hidden' }}>
                      {l.logoUrl ? <img loading="lazy" decoding="async" src={l.logoUrl} alt={l.name} style={{ width: '100%', height: '100%', objectFit: 'contain' }} /> : <span style={{ fontSize: 34 }}>🏢</span>}
                    </div>
                    <div style={{ padding: '11px 12px 14px', minWidth: 0 }}>
                      {l.category && <div style={{ fontFamily: 'var(--font-nunito)', fontSize: 10, fontWeight: 900, color: 'var(--orange)', textTransform: 'uppercase', letterSpacing: 0.4, overflowWrap: 'anywhere' }}>{trCats[idx] || l.category}</div>}
                      <div style={{ fontFamily: 'var(--font-nunito)', fontSize: 14.5, fontWeight: 900, color: 'var(--dark)', marginTop: 2, overflowWrap: 'anywhere' }}>{trNames[idx] || l.name}</div>
                      {l.location && <Place wrap style={{ fontFamily: 'var(--font-nunito)', fontSize: 11, color: 'var(--ink-2)', marginTop: 2 }}>{l.location}</Place>}
                      {l.description && <div style={{ fontFamily: 'var(--font-nunito)', fontSize: 11.5, color: '#1a1a1a', marginTop: 6, lineHeight: 1.4, overflowWrap: 'anywhere', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>{trDescs[idx] || l.description}</div>}
                    </div>
                  </div>
                </Link>
              ))}
